        ...
        try:
            ...
            if (category_id == 0):
                questions = Question.query.all()
                 ...
                 # this means a category was selected so diplay  questions in category selected
                questions = Question.query.filter_by(
                    category=category_id).all()
                formatted_question=[question.format() for question in questions if  question.id not in previous_questions]
                random_question=random.sample(formatted_question, len(formatted_question))
